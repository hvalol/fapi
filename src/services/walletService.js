...
class WalletService {
  ...
  async getWallets({ agentId, clientId }) {
    ...
    const missingTypes = supportedTypes.filter(type => !existingTypes.includes(type));
  ...
  async createWallet({ agentId, clientId, currency, walletType }) {
    if (!agentId || !clientId || !currency || !walletType)
      throw new AppError(
        "agentId, clientId, currency, and walletType are required",
        400
      );
    let wallet = await AgentWallet.findOne({
      where: {
